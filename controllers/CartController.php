...
//            debug($order);
            if ($order->save()){
                ...
                Yii::$app->mailer->compose('order', ['session' => $session])
                    ->setFrom(['armashkaise@mail.ru' => 'Администрация сайта'])
                    ->setTo($order->email)
                    ->setSubject('Заказ с сайта')
                    ->send();

                Yii::$app->mailer->compose('order', ['session' => $session, 'order' => $order])
                    ->setFrom('armashkaise@mail.ru')
                    ->setTo(Yii::$app->params['adminEmail'])
                    ->setSubject('Новый заказ для администратора')
                    ->send();
                //************************************************************КОНЕЦ ОТПРАВКА ПОЧТЫ
                $session->remove('cart');
                $session->remove('cart.qty');
                $session->remove('cart.sum');
                Yii::$app->session->setFlash('success', 'Ваш заказ принят. Наш менеджер в ближайшее время свяжется с Вами.');
                return $this->refresh();
            } else {
